...
import { fetchMegaDetail } from "@/lib/pokemon";
...
export async function generateMetadata({
  ...
  try {
    const pokemon = await fetchMegaDetail(name);
    return {
      title: `${formatName(pokemon.name)} - Pokemon Dex`,
...
export default async function MegaDetailPage({
  params,
}: {
  params: Promise<{ name: string }>;
}) {
  ...
  let pokemon;
  try {
    pokemon = await fetchMegaDetail(name);
  } catch {
    return (
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950">
      <div className="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        {/* Hero section */}
        <div
          className="mb-8 rounded-2xl border border-purple-200 bg-white p-6 shadow-sm ring-2 ring-purple-400/40 dark:border-purple-800 dark:bg-gray-900 dark:ring-purple-500/30 animate-slideUp"
          ...
          <div className="flex flex-col items-center gap-6 sm:flex-row">
            ...
            <div className="flex-1 text-center sm:text-left">
              <div className="flex items-center justify-center gap-2 sm:justify-start">
                <span className="bg-purple-500 text-white text-[10px] font-bold uppercase px-2 py-0.5 rounded-full tracking-wide">
                  Mega
                </span>
