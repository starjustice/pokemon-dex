...
The system SHALL allow filtering by region using colored chip buttons.

#### Scenario: Filter by region
- **WHEN** user selects "Kanto" chip
- **THEN** grid shows only Kanto locations

#### Scenario: Deselect region
- **WHEN** user clicks active region chip
- **THEN** filter clears, showing all regions
...
- **THEN** inline filters hidden, FAB visible; tapping FAB opens sheet with search, region chips, encounters toggle
